...
export class RealtimeGateway implements OnModuleDestroy {
  ...
  handleConnection(client: WebSocket) {
    ...
    this.subscriptions.set(client, { codes: new Set(), types: new Set() })
    client.on('error', (error) => this.logger.debug(`WebSocket error: ${error.message}`))
    this.send(client, { type: 'connected', channel: 'market', realtime: this.realtime.isEnabled(), timestamp: Date.now() })
  }
  ...
  private matches(event: MarketEvent, subscription?: Subscription) {
    ...
    return (!subscription.codes.size || (code ? subscription.codes.has(code) : false)) && (!subscription.types.size || subscription.types.has(type) || subscription.types.has(event.channel.toLowerCase()))
  }
  private send(client: WebSocket, value: unknown) { try { client.send(JSON.stringify(value)) } catch { this.handleDisconnect(client) } }
  private checkConnections() { for (const client of this.clients) { if (client.readyState !== 1) { this.handleDisconnect(client); continue } client.ping() } }
}
