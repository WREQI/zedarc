...
export class NotificationsService implements OnModuleInit, OnModuleDestroy {
  ...
  async markRead(userId: string, id?: string) {
    if (this.database.db) try { await this.database.db.update(notifications).set({ readAt: new Date() }).where(id ? and(eq(notifications.userId, userId), eq(notifications.id, id)) : eq(notifications.userId, userId)); return { updated: true } } catch (error) { if (process.env.NODE_ENV === 'production' && process.env.DEMO_MODE !== 'true') throw error }
  ...
  async add(userId: string, input: { type: string; title: string; content: string }) {
    if (this.database.db) try { const [row] = await this.database.db.insert(notifications).values({ userId, ...input }).returning(); return { ...row, createdAt: row.createdAt.toISOString(), readAt: null } } catch (error) { if (process.env.NODE_ENV === 'production' && process.env.DEMO_MODE !== 'true') throw error }
