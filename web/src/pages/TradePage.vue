...
async function submitOrder() {
  ...
  if (!selectedStock.value || Number(price.value) <= 0 || quantity.value < 100 || quantity.value % 100 !== 0) { showToast('请输入有效价格，数量需为 100 股的整数倍'); return }
  ...
  try {
    if (apiMode.value) {
      try {
        const order = await placeTrade({ code: selectedStock.value.code, side: tradeSide.value, quantity: quantity.value, price: Number(price.value), requestId: requestId() } as Parameters<typeof placeTrade>[0] & { requestId: string })
    ...
    } else if (currentHolding.value) { availableCash.value += estimatedAmount.value; currentHolding.value.quantity -= quantity.value; currentHolding.value.marketValue = (currentHolding.value.quantity * Number(price.value)).toFixed(2) }
    persistDemo(); showToast(`${selectedStock.value.name} ${tradeSide.value === 'buy' ? '买入' : '卖出'}委托已提交`)
