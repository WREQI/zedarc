...
const members = computed(() => {
  ...
  return [...source].sort((a, b) => rank.value === '跌幅榜' ? a.changePercent - b.changePercent : rank.value === '涨幅榜' ? b.changePercent - a.changePercent : 0)
})
const advances = computed(() => detail.value?.members.filter((item) => item.change > 0).length ?? null)
const declines = computed(() => detail.value?.members.filter((item) => item.change < 0).length ?? null)
...
<template>
  <section class="detail-page">
    ...
    <LoadingState v-if="loading" label="正在加载板块详情" />
    <ErrorState v-else-if="error" title="板块详情加载失败" :message="error" :retry="load" />
