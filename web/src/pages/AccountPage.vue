...
async function loadMessages() {
  ...
  try { messages.value = await getNotifications(); unreadMessages.value = messages.value.filter((item) => !item.readAt).length } catch { /* anonymous demo keeps local shell */ }
}
...
function handleMenu(title: string) {
  ...
  if (title === '消息中心') { showMessages.value = true; return }
...
async function markAllRead() {
  if (!auth.user || !unreadMessages.value) return
  try { await markNotificationsRead(); messages.value.forEach((message) => { message.readAt = new Date().toISOString() }); unreadMessages.value = 0; showToast('已全部标记为已读') }
