...
import { getNewsArticle, getNewsArticles, isNewsFavoriteAuthError } from '@/services/news'
...
async function loadArticle() {
  ...
  try {
    ...
    relatedReports.value = current.codes?.length ? (await getReportsPage({ code: current.codes[0], page: 1, pageSize: 5 })).items : []
...
async function toggleSave() {
  const current = article.value
  if (!current) return
  try { await favorites.toggle(current.id); saved.value = favorites.has(current.id) }
  ...
  <section class="news-detail-page">
    ...
    <template v-else-if="article">
      ...
      <article class="article-card">
        ...
        <button v-if="article.videoUrl" class="detail-video" :class="{ playing: videoPlaying }" @click="toggleVideo"><span>{{ videoPlaying ? 'Ⅱ' : '▶' }}</span><strong>{{ videoPlaying ? '正在播放视频' : '播放视频' }}</strong><small>点击播放</small><i v-if="videoPlaying" /></button>
        <EmptyState v-else-if="article.video" title="视频内容暂不可用" message="资讯接口标记了视频类型，但没有返回可播放地址。" icon="▶" />
      ...
      <section class="related-news-panel">
        ...
        <RouterLink v-for="item in related" :key="item.id" :to="`/news/${item.id}`" class="related-news-row"><span class="news-tag">{{ item.tag }}</span><strong>{{ item.title }}</strong><time>{{ item.time || item.publishedAt?.slice(11, 16) }}</time><span class="result-arrow">›</span></RouterLink>
        <EmptyState v-if="!related.length" title="暂无相关阅读" message="当前接口未返回可关联的资讯。" />
      </section>
      <section v-if="article.codes?.length" class="related-reports-panel">
        <div class="block-title"><h2>相关研报</h2><RouterLink to="/reports" class="text-button">更多 ›</RouterLink></div>
        <RouterLink v-for="report in relatedReports" :key="report.id" :to="`/reports/${report.id}`" class="related-report-row"><strong>{{ report.title }}</strong><span>{{ report.institution }} · {{ report.rating }} ›</span></RouterLink>
        <EmptyState v-if="!relatedReports.length" title="暂无相关研报" message="当前研报接口未返回该标的的真实研报。" />
      </section>
    </template>
...
<style scoped>
.news-detail-page { width: min(720px, 100%); min-height: 100%; margin: 0 auto; color: var(--text); }.toast { position: fixed; bottom: 80px; left: 50%; z-index: 10; transform: translateX(-50%); padding: 10px 15px; border-radius: 5px; background: rgba(38,46,64,.9); color: #fff; font-size: 11px; white-space: nowrap; }
...
.related-news-panel { margin-top: 9px; padding: 18px 16px 26px; background: var(--card); }.related-reports-panel { margin-top: 9px; padding: 18px 16px 26px; background: var(--card); }.related-report-row { display: flex; justify-content: space-between; gap: 12px; padding: 13px 0; border-bottom: 1px solid var(--border); color: var(--text); font-size: 11px; }.related-report-row span { color: var(--muted); white-space: nowrap; }.block-title { display: flex; align-items: center; justify-content: space-between; margin-bottom: 3px; }.block-title h2 { font-size: 15px; font-weight: 600; }.text-button { color: var(--primary); font-size: 11px; }.related-news-row { display: grid; grid-template-columns: 45px minmax(0, 1fr) 32px 14px; gap: 8px; align-items: center; padding: 13px 0; border-bottom: 1px solid var(--border); }.related-news-row strong { overflow: hidden; font-size: 12px; font-weight: 400; line-height: 1.45; text-overflow: ellipsis; white-space: nowrap; }.related-news-row time { color: var(--muted); font: 10px 'JetBrains Mono', monospace; }.result-arrow { color: #b8bfca; font-size: 19px; }
