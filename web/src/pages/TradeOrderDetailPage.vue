...
const timeline = ref<Array<{ eventId: string; status: TradeOrderEventStatus; timestamp: number }>>([])
let disconnectRealtime: (() => void) | undefined
const statusText = (status?: string) => status === 'cancelled' || status === '已撤' ? '已撤' : status === 'filled' || status === '已成' ? '已成' : status === 'placed' || status === '已报' ? '已报' : status || '暂无数据'
