...
import { cancelTrade, getTradeOrders, isTradeOrderCancellable, loadDemoAccount, saveDemoAccount, tradeStatusText, type DemoOrder } from '@/services/trade'
...
interface PageOrder extends DemoOrder {
  id?: string
...
const cancelRetryOrder = ref<PageOrder | null>(null)
...
const statusFilter = (order: PageOrder) => order.status
const filteredOrders = computed(() => activeFilter.value === '全部' ? orders.value : orders.value.filter((order) => statusFilter(order) === activeFilter.value))
...
function displayStatus(status: string) { return tradeStatusText(status) }
function toPageOrder(order: { id: string; code: string; side: 'buy' | 'sell'; price: number; quantity: number; status: string; statusReason?: string | null; statusUpdatedAt?: string; requestId?: string | null; createdAt: string }): PageOrder {
  return { id: order.id, requestId: order.requestId, rawStatus: order.status, statusReason: order.statusReason, statusUpdatedAt: order.statusUpdatedAt, time: new Date(order.createdAt).toLocaleTimeString('zh-CN', { hour12: false }).slice(0, 8), name: stocks.find((stock) => stock.code === order.code)?.name ?? order.code, side: order.side === 'buy' ? '买入' : '卖出', price: order.price.toFixed(2), quantity: order.quantity, status: displayStatus(order.status) }
}
...
function applyRealtimeEvent(event: TradeOrderRealtimeEvent) {
  ...
  if (existing) {
    existing.status = displayStatus(event.status)
    existing.rawStatus = event.status
    existing.statusReason = event.reason ?? event.order.statusReason
    existing.statusUpdatedAt = event.order.statusUpdatedAt ?? new Date(event.timestamp).toISOString()
    return
  }
  orders.value.unshift(toPageOrder(event.order))
}

async function retryCancel() { if (cancelRetryOrder.value) await cancelOrder(cancelRetryOrder.value) }

async function cancelOrder(order: PageOrder) {
  if (!isTradeOrderCancellable(order.status)) return
  if (apiMode.value && order.id) {
    try {
      const cancelled = await cancelTrade(order.id)
      order.status = displayStatus(cancelled.status)
      order.statusReason = cancelled.statusReason
      await refreshRemoteOrders()
      cancelRetryOrder.value = null
      cancelError.value = ''
      showToast(`${order.name} 委托已撤销，状态已刷新`)
      return
    } catch (error) {
      cancelRetryOrder.value = order
...
<template>
  <section class="trade-page">
    ...
    <template v-else>
      <div v-if="loadError" class="error-banner"><b>!</b><span>{{ loadError }}</span><button type="button" @click="loadError = ''">×</button><button type="button" @click="refreshRemoteOrders">重试</button></div><div v-if="cancelError" class="error-banner"><b>!</b><span>{{ cancelError }}</span><button type="button" @click="retryCancel">重试撤单</button></div><div v-if="apiMode" class="realtime-banner"><i />{{ realtimeStatus === 'connected' ? '订单状态实时同步' : realtimeStatus === 'reconnecting' || realtimeStatus === 'connecting' ? '正在恢复实时连接…' : '实时连接已断开，HTTP 快照仍可用' }}</div>
      <section class="list-card"><div class="section-title"><div><h2>今日委托</h2><small>交易日 · 实时状态</small></div><span>{{ filteredOrders.length }} / {{ orders.length }} 条</span></div><div class="filter-tabs"><button v-for="filter in filters" :key="filter" :class="{ active: activeFilter === filter }" @click="activeFilter = filter">{{ filter }}</button></div><div v-for="(order, index) in filteredOrders" :key="order.id ?? `${order.time}-${order.name}-${index}`" class="order-row"><div><RouterLink :to="`/trade/orders/${order.id ?? index}`" class="detail-link"><b>{{ order.name }} <em :class="order.side === '买入' ? 'text-up' : 'text-down'">{{ order.side }}</em></b><small>{{ order.time }} · {{ order.price }} 元 × {{ order.quantity }} 股</small><small v-if="order.statusReason" class="status-reason">{{ order.statusReason }}</small></RouterLink></div><div class="order-actions"><span class="status" :class="{ cancelled: order.status === '已撤' || order.status === '已拒', done: order.status === '已成', partial: order.status === '部分成交' }">{{ order.status }}</span><button v-if="isTradeOrderCancellable(order.status)" @click="cancelOrder(order)">撤单</button><RouterLink :to="`/trade/orders/${order.id ?? index}`" class="detail-action">详情 ›</RouterLink></div></div><p v-if="!filteredOrders.length" class="empty-state">暂无{{ activeFilter === '全部' ? '' : activeFilter }}委托<br /><small>提交交易后将在这里展示</small></p></section>
