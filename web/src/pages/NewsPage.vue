...
const filteredNews = computed(() => {
  ...
  const categoryNews = activeCategory.value === '全部'
    ? source
    : source.filter((item) => item.tag === activeCategory.value || (activeCategory.value === '市场' && item.tag === '市场热点'))
  const search = keyword.value.trim().toLowerCase()
  return search
    ? categoryNews.filter((item) => `${item.title} ${item.summary} ${item.source}`.toLowerCase().includes(search))
    : categoryNews
})
  ...
  <section class="news-page">
    <header class="news-header">
      ...
      <label class="search-box">
        ...
        <input v-model="keyword" type="search" placeholder="搜索资讯、作者或来源" aria-label="搜索资讯">
    ...
    <div class="news-date"><span>今日资讯</span><time>2026年08月10日</time></div>
...
.search-box input::placeholder { color: #aab2bf; }.search-box button { border: 0; background: transparent; color: var(--muted); font-size: 18px; }
.category-bar { display: flex; gap: 22px; overflow-x: auto; padding: 0 16px; scrollbar-width: none; }.category-bar::-webkit-scrollbar { display: none; }
.category-bar button { position: relative; flex: 0 0 auto; padding: 10px 0 11px; border: 0; background: transparent; color: var(--muted); font-size: 13px; white-space: nowrap; }.category-bar button.selected { color: var(--primary); font-weight: 600; }.category-bar button.selected::after { position: absolute; right: 50%; bottom: 0; left: 50%; height: 2px; border-radius: 2px; background: var(--primary); content: ''; transform: translateX(-50%); }.category-bar button span { margin-left: 3px; font-size: 10px; }
