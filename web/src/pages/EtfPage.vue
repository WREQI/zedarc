...
const sortTabs = sortOptions.map((item: Sort) => ({ label: item, value: item }))
const categoryTabs = categoryOptions.map((item: Category) => ({ label: item, value: item }))
...
<template>
  <section class="etf-page"><PageHeader eyebrow="MARKET / ETF" title="ETF行情" description="按名称、代码和分类筛选，排序仅使用 provider 已返回的字段。"><template #actions><button class="secondary-button" type="button" @click="load">刷新</button></template></PageHeader>
    <section class="toolbar panel"><div class="toolbar-line"><HorizontalTabs :items="sortTabs" :model-value="sort" aria-label="ETF排序" @update:model-value="sort = $event as Sort" /><label class="search">⌕<input v-model="keyword" type="search" placeholder="搜索 ETF 名称或代码" /></label></div><HorizontalTabs :items="categoryTabs" :model-value="category" aria-label="ETF分类" @update:model-value="category = $event as Category" /></section>
